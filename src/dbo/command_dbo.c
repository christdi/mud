...
int select_commands_by_name(game_t* game, const char* name, linked_list_t* commands) {
  ...
  if (sqlite3_prepare_v2(game->database, sql, -1, &res, 0) != SQLITE_OK) {
    mlog(ERROR, "select_commands_by_name", "Failed to prepare statement to retrieve commands from database: [%s]", sqlite3_errmsg(game->database));
  ...
  if (sqlite3_bind_text(res, 1, name, (int)strlen(name), NULL) != SQLITE_OK) {
    mlog(ERROR, "select_commands_by_name", "Failed to bind command name to retrieve commands from database: [%s]", sqlite3_errmsg(game->database));
    sqlite3_finalize(res);
  ...
  while ((rc = sqlite3_step(res)) != SQLITE_DONE) {
    if (rc != SQLITE_ROW) {
      mlog(ERROR, "select_commands_by_name", "Failed to retreive commands from database: [%s]", sqlite3_errmsg(game->database));
    ...
    if (strlcpy(command_dbo->name, (char*)sqlite3_column_text(res, 0), COMMAND_NAME_LENGTH) > COMMAND_NAME_LENGTH) {
      mlog(WARN, "select_commands_by_name", "Error retrieving commands from database. Command name length for command [%s] was longer than max length [%d] and was truncated", name, COMMAND_NAME_LENGTH);
    }
