...
int network_server_listen(server_t *server) {
  ...
  if (!portString) {
    zlog_error(networkCategory,
               "Failed to convert port from integer to string.");
...
int network_server_create_thread(server_t *server) {
  ...
  if (pthread_create(&server->thread, NULL, network_server_accept_thread,
                     server) != 0) {
    zlog_error(networkCategory, "%s", strerror(errno));
  ...
  while (server->fd > 0) {
    ...
    if (results == -1) {
      ...
      if (FD_ISSET(server->fd, &readSet)) {
        ...
        client->fd = accept(server->fd, (struct sockaddr *)&remoteAddress,
                            &remoteAddressSize);
        ...
        zlog_info(networkCategory, "New client with descriptor [%d]",
                  client->fd);
...
int network_server_poll_clients(server_t *server) {
  ...
  while (node != NULL) {
    ...
    if (client->hungup == 1) {
      zlog_info(networkCategory, "Client descriptor [%d] disconnected",
                client->fd);
...
int network_server_close(server_t *server) {
  ...
  if (server->fd) {
    zlog_info(networkCategory,
              "Closing server on port [%d] with descriptor [%d]", server->port,
              server->fd);
